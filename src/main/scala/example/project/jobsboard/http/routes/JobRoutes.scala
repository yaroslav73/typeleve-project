...
class JobRoutes[F[_]: Monad] private extends Http4sDsl[F]:
  ...
  private val createJobRoute: HttpRoutes[F] = HttpRoutes.of[F] { case POST -> Root / "new" =>
  ...
  val routes: HttpRoutes[F] = Router("/jobs" -> (allJobsRoute <+> findJobRoute <+> createJobRoute <+> updateJobRoute <+> deleteJobRoute))
